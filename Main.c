...
int *calculatePlayOrder() {
  // Add players to result by randomly hopping through the list, changing a
  // used number to -1 to show it has been selected
...
void runGameLoop() {
  ...
  while (numberOfAlivePlayers) {
    ...
    printf("Thats the end of round %d, here are the scores...", roundNumber);
    for (int i = 0; i < numberOfPlayers; i++) {
...
void displayResults() {
  ...
  if (gameTied) {
    printf("There are no winners today, you're all losers!\n");
